...
if (Test-Path -LiteralPath $dockerDisk) {
    ...
    if ($freeBytes -lt 40GB) {
        throw ('Docker data drive {0}: has only {1:N1} GiB free; Stage B2 requires at least 40 GiB before starting.' -f $dockerDrive, ($freeBytes / 1GB))
    }
}
...
docker run --rm `
    ...
    --mount $volumeMount `
    $imageName `
    /bin/sh -c 'PHANTOWD_PREPARE_ONLY=1 /external/support/container/build-qemu.sh && /external/support/container/build-ex4-stage-b2.sh'
if ($LASTEXITCODE -ne 0) { throw 'The verified-source preparation or Stage B2 compile failed.' }
