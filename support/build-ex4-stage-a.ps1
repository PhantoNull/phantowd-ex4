...
docker run --rm `
    ...
    --mount $volumeMount `
    $imageName `
    /bin/sh -c '/external/support/container/build-qemu.sh && /external/support/container/build-ex4-stage-a.sh'

if ($LASTEXITCODE -ne 0) {
    throw 'The QEMU prerequisite or EX4 Stage A compile failed.'
}
