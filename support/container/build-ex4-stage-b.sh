...
config_file="$external_dir/configs/phantowd_ex4_stage_b_defconfig"
fragment_file="$stage_b_dir/linux.fragment"
dts_file="$stage_b_dir/kirkwood-wd-mycloud-ex4-stage-b.dts"
init_file="$stage_b_dir/rootfs-overlay/init"
release_file="$stage_b_dir/rootfs-overlay/etc/phantowd-release"
...
grep -Fx 'PHANTOWD_NETWORK_MODE=enumeration-only' "$release_file" >/dev/null
...
for node in uart0 eth0 mdio; do assert_dts_status "$node" okay; done
for node in uart1 gpio0 gpio1 nand crypto_sram sata sata_phy0 sata_phy1 \
    sdio usb0 pciec pcie0 pcie1 eth1 i2c0 i2c1 spi0 rtc thermal wdt \
...
input_hash="$(
    sha256sum "$config_file" "$stage_a_dir/linux.fragment" \
        ...
        "$fragment_file" "$dts_file" "$init_file" "$release_file" |
        sha256sum | cut -c1-16
)"
output_dir="$workspace_dir/stage-b/$BUILDROOT_VERSION-$input_hash"
...
dtb="$output_dir/images/kirkwood-wd-mycloud-ex4-stage-b.dtb"
...
# Stage B must retain the same exact minimal BusyBox surface as Stage A.
...
test -x "$mkimage"
wrapped="$output_dir/uImage-stage-b.compile-only"
SOURCE_DATE_EPOCH=0 "$mkimage" -A arm -O linux -T kernel -C none \
    -a 0x00008000 -e 0x00008000 \
    -n 'PhantoWD EX4 Stage B' -d "$appended" "$wrapped"
wrapped_size="$(wc -c < "$wrapped" | tr -d '[:space:]')"
test "$wrapped_size" -eq "$((appended_size + 64))"
test "$wrapped_size" -le 5242880
SOURCE_DATE_EPOCH=0 "$mkimage" -A arm -O linux -T kernel -C none \
    -a 0x00008000 -e 0x00008000 \
    -n 'PhantoWD EX4 Stage B' -d "$appended" \
    "$output_dir/uImage-stage-b-second-build.actual" >/dev/null
cmp "$wrapped" "$output_dir/uImage-stage-b-second-build.actual"
...
artifact_dir="$external_dir/artifacts/ex4-stage-b-compile-only"
install -d -m 0755 "$artifact_dir"
install -m 0644 "$zimage" "$artifact_dir/zImage"
install -m 0644 "$dtb" "$artifact_dir/kirkwood-wd-mycloud-ex4-stage-b.dtb"
install -m 0644 "$appended" "$artifact_dir/zImage-with-appended-dtb.compile-only"
install -m 0644 "$wrapped" "$artifact_dir/uImage-stage-b.compile-only"
...
Stage B is a NON-FLASHABLE research artifact. Do not boot it before a separate
physical safety review. It adds only Ethernet 0 and MDIO/PHY enumeration to
Stage A. The interface remains DOWN; there is no DHCP, IP configuration,
packet-generating userspace applet, SSH server, storage, MTD or NAND writer.
The first Stage A boot does not qualify Stage B hardware, cooling or recovery.
EOF
...
stage=B-ethernet-enumeration
network_mode=enumeration-only
storage_enabled=no
...
(
    cd "$artifact_dir"
    sha256sum zImage kirkwood-wd-mycloud-ex4-stage-b.dtb \
        zImage-with-appended-dtb.compile-only uImage-stage-b.compile-only \
        linux.config rootfs.cpio COMPILE-ONLY.txt \
        UIMAGE-RESEARCH-MANIFEST.txt > SHA256SUMS
)
printf 'EX4 Stage B compile passed. Non-flashable artifact: %s\n' "$artifact_dir"
