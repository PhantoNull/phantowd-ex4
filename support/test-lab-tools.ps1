...
$moduleRoot = Join-Path $repoRoot 'tools/phantowd-lab'
$reportRoot = Join-Path $repoRoot 'artifacts/lab-tools-host-tests'
...
try {
    ...
    go vet ./...
    if ($LASTEXITCODE -ne 0) { throw 'Offline toolkit static checks failed.' }
    go test -count=1 "-coverprofile=$coverageFile" ./...
    if ($LASTEXITCODE -ne 0) { throw 'Offline toolkit tests failed.' }
} finally {
    Pop-Location
    foreach ($environmentName in $environmentNames) {
