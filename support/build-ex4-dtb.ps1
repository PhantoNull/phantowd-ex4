...
docker run --rm `
    ...
    --mount $volumeMount `
    $imageName `
    /bin/sh -c '/external/support/container/build-qemu.sh && /external/support/container/build-ex4-dtb.sh'

if ($LASTEXITCODE -ne 0) {
    throw 'The ARMv5 baseline or EX4 device-tree compile failed.'
}
